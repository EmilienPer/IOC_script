...
class LokiLogger:
    ...
    def __init__(self, no_log_file, log_file, hostname, remote_host, remote_port, syslog_tcp, csv, only_relevant, debug,
                 platform, caller, customformatter=None):
    ...
    def log_to_stdout(self, message, mes_type):
        ...
        if self.csv:
            print(
                self.Format(self.STDOUT_CSV, '{0},{1},{2},{3}', getSyslogTimestamp(), self.hostname, mes_type, message))
    ...
    def log_to_file(self, message, mes_type, module):
        try:
            ...
            with codecs.open(self.log_file, "a", encoding='utf-8') as logfile:
                if self.csv:
                    logfile.write(
                        self.Format(self.FILE_CSV, u"{0},{1},{2},{3},{4}{5}", getSyslogTimestamp(), self.hostname,
                                    mes_type, module, message, self.linesep))
                else:
                    logfile.write(self.Format(self.FILE_LINE, u"{0} {1} LOKI: {2}: MODULE: {3} MESSAGE: {4}{5}",
                                              getSyslogTimestamp(), self.hostname, mes_type.title(), module, message,
                                              self.linesep))
        except Exception as e:
            if self.debug:
                traceback.print_exc()
    ...
    def log_to_remotesys(self, message, mes_type, module):
        ...
        syslog_message = self.Format(self.SYSLOG_LINE, "LOKI: {0}: MODULE: {1} MESSAGE: {2}", mes_type.title(), module,
                                     message)
        try:
