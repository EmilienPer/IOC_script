...
class VulnChecker():
    ...
    def check_sam_readable(self):
        ...
        output = b''
        try:
            output += subprocess.check_output([r'icacls.exe', r'C:\Windows\System32\config\sam'],
                                              stderr=subprocess.STDOUT)
        except subprocess.CalledProcessError as e:
            pass
        try:
            output += subprocess.check_output([r'icacls.exe', r'C:\Windows\SysNative\config\sam'],
                                              stderr=subprocess.STDOUT)
        except subprocess.CalledProcessError as e:
            pass
        # Check the output
