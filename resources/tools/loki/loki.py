...
class Loki(object):
    ...
    def __init__(self, intense_mode):
        ...
        logger.log("INFO", "Init",
                   "Malicious SHA256 Hashes initialized with %s hashes" % len(self.hashes_sha256.keys()))
        ...
        self.initialize_filetype_magics(
            os.path.join(self.app_path, 'signature-base/misc/file-log_type-signatures.txt'.replace("/", os.sep)))
            ...
            for filename in files:
                try:
                    ...
                    if do_intense_check:
                        if args.printall:
                            logger.log("INFO", "FileScan",
                                       "Scanning %s TYPE: %s SIZE: %s" % (fileNameCleaned, fileType, fileSize))
                    else:
                        if args.printall:
                            logger.log("INFO", "FileScan",
                                       "Checking %s TYPE: %s SIZE: %s" % (fileNameCleaned, fileType, fileSize))
                    ...
                    if do_intense_check:
                        ...
                        if args.scriptanalysis:
                            if extension in SCRIPT_EXTENSIONS or type in SCRIPT_TYPES:
                                logger.log("DEBUG", "FileScan",
                                           "Performing character analysis on file %s ... " % filePath)
                        ...
                        if fileType == "MDMP":
                            logger.log("INFO", "FileScan",
                                       "Scanning memory dump file %s" % fileNameCleaned.decode('utf-8'))
    ...
    def scan_processes(self, nopesieve, nolisten, excludeprocess, pesieveshellc):
        ...
        for process in processes:
            ...
            if processExists(pid):
                if int(ws_size) < (args.maxworkingset * 1048576):
                    try:
                        ...
                        if len(alerts) > 5:
                            logger.log("WARNING", "ProcessScan",
                                       "Too many matches on process memory - most likely a false positive %s" % process_info)
                        elif len(alerts) > 0:
                            for alert in alerts:
                        ...
                        if path != "none":
                            logger.log("ERROR", "ProcessScan",
                                       "Error during process memory Yara check (maybe the process doesn't exist anymore or access denied) %s" % process_info)
                else:
                    logger.log("DEBUG", "ProcessScan",
                               "Skipped Yara memory check due to the process' big working set size (stability issues) PID: %s NAME: %s SIZE: %s" % (
                               pid, name, ws_size))
                ...
                if processExists(pid) and self.peSieve.active and not nopesieve:
                    ...
                    if results["replaced"]:
                        ...
                        logger.log("WARNING", "ProcessScan", "PE-Sieve reported implanted process %s "
                                                             "IMPLANTED PE: %s IMPLANTED SHC: %s" % (
                                   process_info, str(results["implanted_pe"]),
                        ...
                        logger.log("NOTICE", "ProcessScan",
                                   "PE-Sieve reported a process with unreachable exe %s UNREACHABLE: %s" %
            ...
            if name == "lsm.exe" and not (
                    owner.startswith("NT ") or owner.startswith("LO") or owner.startswith("SYSTEM") or owner.startswith(
                    u"система")):
                ...
                if x.status == 'ESTABLISHED':
                    ...
                    logger.log("NOTICE", "ProcessScan",
                               "Established connection PID: %s NAME: %s COMMAND: %s LIP: %s LPORT: %s RIP: %s RPORT: %s" % (
                                   str(pid), name, command, str(x.laddr[0]), str(x.laddr[1]), str(x.raddr[0]),
                                   str(x.raddr[1])))
                ...
                if 'hash' in ioc_filename:
                        ...
                        for line in lines:
                            try:
                                ...
                                if hash in HASH_WHITELIST:
                                    continue
                                # Else - check which log_type it is
                                if len(hash) == 32:
                    ...
                    if logger.debug:
                        logger.log("DEBUG", "Init", "Initialized %s hash IOCs from file %s"
                                   % (str(len(self.hashes_md5) + len(self.hashes_sha1) + len(self.hashes_sha256)),
                                      ioc_filename))
...
def main():
    ...
    parser.add_argument('-s', help='Maximum file size to check in KB (default 5000 KB)', metavar='kilobyte',
                        default=5000)
    ...
    parser.add_argument('--allhds', action='store_true', help='Scan all local hard drives (Windows only)',
                        default=False)
    ...
    parser.add_argument('--allreasons', action='store_true', help='Print all reasons that caused the score',
                        default=False)
    ...
    parser.add_argument('--csv', action='store_true', help='Write CSV log format to STDOUT (machine processing)',
                        default=False)
    ...
    parser.add_argument('--logfolder', help='Folder to use for logging when log file is not specified',
                        metavar='log-folder', default='')
    ...
    if args.logfolder and args.l:
        print(
            'Must specify either log folder with --logfolder, which uses the default filename, or log file with -l. Log file can be an absolute path')
        sys.exit(1)
    elif args.logfolder:
...
if __name__ == '__main__':
    ...
    logger = LokiLogger(args.nolog, args.l, getHostname(os_platform), args.r, int(args.t), args.syslogtcp, args.csv,
                        args.onlyrelevant, args.debug,
    ...
    if os_platform == "windows":
        if shell.IsUserAnAdmin():
            ...
            logger.log("NOTICE", "Init",
                       "Program should be run 'as Administrator' to ensure all access rights to process memory and file objects.")
    else:
        if os.geteuid() == 0:
            isAdmin = True
    ...
    if logger.alerts:
        ...
        logger.log("RESULT", "Results",
                   "Loki recommends checking the elements on virustotal.com or Google and triage with a "
    ...
    logger.log("NOTICE", "Results",
               "Finished LOKI Scan SYSTEM: %s TIME: %s" % (getHostname(os_platform), getSyslogTimestamp()))
