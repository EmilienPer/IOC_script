...
def ScanWMI():
    if os_platform == "windows":
        ...
        knownHashes = ['159e2bcde798cf5fbb290f90a7ccc1a6', '20d385446e60cf9134792d5b145c54bb',
                       '65c80cb7a9094b32c3f9982887b9862a', '6ddb270d17551138747ad7c1bc3db9b3',
                       'de5b1c4f59c4463f8e9b70cbe1156976']
        ...
        for eventFilter in leventFilter:
            try:
                ...
                if not hashEntry in knownHashes:
                    logger.log("WARNING", "WMIScan", 'CLASS: __eventFilter MD5: %s NAME: %s QUERY: %s' % (
                    hashEntry, eventFilter.wmi_property('Name').value, eventFilter.wmi_property('Query').value))
            except:
        ...
        for FilterToConsumerBinding in lFilterToConsumerBinding:
            try:
                ...
                if not hashEntry in knownHashes:
                    logger.log("WARNING", "WMIScan",
                               'CLASS: __FilterToConsumerBinding MD5: %s CONSUMER: %s FILTER: %s' % (
                               hashEntry, FilterToConsumerBinding.wmi_property('Consumer').value,
                               FilterToConsumerBinding.wmi_property('Filter').value))
            except:
        ...
        for CommandLineEventConsumer in lCommandLineEventConsumer:
            try:
                ...
                if not hashEntry in knownHashes:
                    logger.log("WARNING", "WMIScan",
                               'CLASS: CommandLineEventConsumer MD5: %s NAME: %s COMMANDLINETEMPLATE: %s' % (
                               hashEntry, CommandLineEventConsumer.wmi_property('Name').value,
                               CommandLineEventConsumer.wmi_property('CommandLineTemplate').value))
            except:
